...
class _MeasureSizeCatState extends State<_MeasureSizeCatView> {
  ...
  static const List<String> _artLabels = [
    ...
    'sketch', 'painting', 'digital art', 'manga', 'clipart', 'vector', 'poster',
    'figure', 'figurine', 'toy', 'stuffed animal', 'plush', 'statue', 'sculpture',
    'origami', 'paper craft', 'papercraft', 'paper model', 'paper art', 'model',
    'miniature', 'replica', 'doll', 'puppet', 'mannequin', 'ceramic', 'porcelain',
    'clay', 'plastic', 'wood carving', '3d model', 'render', '3d render', 'cgi',
    'computer graphics', 'craft', 'handmade', 'artifact', 'decorative',
    'low poly', 'polygon', 'geometric', 'object', 'prop',
  ];
  static const List<String> _realAnimalLabels = [
    'fur', 'whisker', 'mammal', 'wildlife', 'fauna', 'paw', 'animal', 'pet',
    'domestic animal', 'nose', 'claw', 'tail', 'coat', 'hair', 'living',
    'vertebrate', 'carnivore',
  ];
  static const List<String> _nonCatAnimalLabels = [
    'otter', 'sea otter', 'river otter', 'mink', 'ferret', 'weasel', 'marten',
    'beaver', 'badger', 'skunk', 'seal', 'sea lion', 'walrus', 'dolphin',
    'whale', 'shark', 'octopus', 'crab', 'lobster', 'fish', 'fox', 'wolf',
    'bear', 'panda', 'raccoon', 'squirrel', 'rabbit', 'hare', 'hamster',
    'guinea pig', 'gerbil', 'rat', 'mouse', 'hedgehog', 'meerkat', 'mongoose',
    'capybara', 'monkey', 'ape', 'chimpanzee', 'gorilla', 'lemur', 'koala',
    'kangaroo', 'deer', 'elk', 'reindeer', 'moose', 'alpaca', 'llama',
    'sheep', 'goat', 'cow', 'horse', 'pig', 'bird', 'parrot', 'owl', 'eagle',
    'chicken', 'duck', 'tiger', 'lion', 'cheetah', 'leopard', 'jaguar', 'lynx',
    'bobcat', 'cougar', 'panther', 'snake', 'lizard', 'turtle', 'frog', 'gecko',
    'iguana', 'chameleon', 'crocodile', 'alligator', 'reptile', 'dinosaur',
    'dragon', 'spider', 'insect', 'scorpion',
  ];
  ...
  Future<bool> _hasMultipleCats(String imagePath) async {
    ...
    try {
      ...
      for (int i = 0; i < regions.length; i++) {
        if (_isDisposed) return false;
        final r = regions[i];
        final cropped = img.copyCrop(image, x: r['x']!, y: r['y']!, width: r['w']!, height: r['h']!);
        final regionFile = File('${tempDir.path}/region_${i}_${DateTime.now().millisecondsSinceEpoch}.jpg');
        ...
        try {
          ...
          if (regionCatScore >= 0.50) catRegionCount++;
        } catch (e) {
  ...
  Future<_DetectionResult> _detectCatPro(String imagePath) async {
    if (_isDisposed) return const _DetectionResult(isCat: false, reason: 'disposed', catScore: 0, isCartoon: false);
    try {
      final inputImage = InputImage.fromFilePath(imagePath);
      final labels = await _imageLabeler.processImage(inputImage);
      if (_isDisposed) return const _DetectionResult(isCat: false, reason: 'disposed', catScore: 0, isCartoon: false);

      double catScore = 0.0, dogScore = 0.0, artScore = 0.0, realAnimalScore = 0.0, nonCatAnimalScore = 0.0;
      String nonCatAnimalName = '';

      for (final label in labels) {
        final text = label.label.toLowerCase();
        final conf = label.confidence;
        for (final l in _catLabels) { if (text.contains(l) && conf > catScore) catScore = conf; }
        for (final l in _dogLabels) { if (text.contains(l) && conf > dogScore) dogScore = conf; }
        for (final l in _artLabels) { if (text.contains(l) && conf > artScore) artScore = conf; }
        for (final l in _realAnimalLabels) { if (text.contains(l) && conf > realAnimalScore) realAnimalScore = conf; }
      ...
      if (catScore < 0.50) return _DetectionResult(isCat: false, reason: 'cat_score_low', catScore: catScore, isCartoon: false);
      if (artScore >= 0.30 && realAnimalScore < 0.55) return _DetectionResult(isCat: false, reason: 'art_suspected_no_real_signal', catScore: catScore, isCartoon: true);
      if (realAnimalScore < 0.45) return _DetectionResult(isCat: false, reason: 'art_suspected_no_real_signal', catScore: catScore, isCartoon: true);
      if (isCartoon && catScore < 0.80) return _DetectionResult(isCat: false, reason: 'cartoon_texture', catScore: catScore, isCartoon: true);

      if (catScore >= 0.60 && realAnimalScore >= 0.40) {
        final hasMultiple = await _hasMultipleCats(imagePath);
        if (_isDisposed) return const _DetectionResult(isCat: false, reason: 'disposed', catScore: 0, isCartoon: false);
        if (hasMultiple) return _DetectionResult(isCat: false, reason: 'multiple_cats', catScore: catScore, isCartoon: false);
        return _DetectionResult(isCat: true, reason: 'real_animal_confirmed', catScore: catScore, isCartoon: false);
      }

      if (artScore >= 0.60) return _DetectionResult(isCat: false, reason: 'art_label_high', catScore: catScore, isCartoon: true);
      if (catScore < 0.65) return _DetectionResult(isCat: false, reason: 'cat_score_low', catScore: catScore, isCartoon: isCartoon);
      if (dogScore >= 0.50 && (catScore - dogScore) < 0.20) return _DetectionResult(isCat: false, reason: 'ambiguous_cat_dog', catScore: catScore, isCartoon: isCartoon);
      if (nonCatAnimalScore >= 0.45) return _DetectionResult(isCat: false, reason: 'non_cat_animal:$nonCatAnimalName', catScore: catScore, isCartoon: false);
      if (nonCatAnimalScore > 0.30 && (catScore - nonCatAnimalScore) < 0.30) return _DetectionResult(isCat: false, reason: 'cat_not_dominant_over_other_animal:$nonCatAnimalName', catScore: catScore, isCartoon: false);
      if (realAnimalScore < 0.35) return _DetectionResult(isCat: false, reason: 'art_suspected_no_real_signal', catScore: catScore, isCartoon: true);

      final hasMultiple = await _hasMultipleCats(imagePath);
      if (_isDisposed) return const _DetectionResult(isCat: false, reason: 'disposed', catScore: 0, isCartoon: false);
      if (hasMultiple) return _DetectionResult(isCat: false, reason: 'multiple_cats', catScore: catScore, isCartoon: false);

      return _DetectionResult(isCat: true, reason: 'passed', catScore: catScore, isCartoon: false);
    } catch (e) {
  ...
  Future<void> _pickImage() async {
    ...
    try {
      final XFile? image = await _picker.pickImage(
          source: ImageSource.gallery, imageQuality: 95, maxWidth: 1920, maxHeight: 1920);
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: const Text('รับทราบ', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          ),
        ],
      ),
    );
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(
            ...
            child: Text(reason, style: TextStyle(fontSize: 14, color: Colors.grey.shade700, height: 1.5), textAlign: TextAlign.center),
          ),
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('ถ่ายใหม่', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          )
        ],
      ),
    );
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    Text(
                      languageProvider.translate(en: 'Added to Favorites', th: 'เพิ่มในรายการโปรด'),
                      style: TextStyle(
                        fontSize: 17,
                  ...
                  children: [
                    Expanded(
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          Navigator.pop(ctx);
                          try {
                            await _basketApi.addToBasket(clothingUuid: product['id']);
                    ...
                    Expanded(
                      ...
                      child: ElevatedButton(
                        ...
                        child: Text(
                            languageProvider.translate(en: 'More', th: 'เพิ่มเติม'),
                            style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      ),
                    ),
                  ],
        ...
        child: StatefulBuilder(
          builder: (ctx2, setModal) => Column(
            ...
            children: [
              ...
              const Text('✏️ แก้ไขข้อมูลแมว', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 16),
              TextField(controller: colorCtrl, decoration: const InputDecoration(labelText: 'สีแมว / Cat Color', border: OutlineInputBorder())),
              const SizedBox(height: 12),
              TextField(controller: breedCtrl, decoration: const InputDecoration(labelText: 'พันธุ์ / Breed', border: OutlineInputBorder())),
              const SizedBox(height: 12),
                ...
                children: ['XS', 'S', 'M', 'L', 'XL'].map((size) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Text(size, style: TextStyle(fontWeight: FontWeight.bold, color: selected ? Colors.white : Colors.black87)),
                    ),
                  );
                }).toList(),
              ),
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          ...
          style: TextStyle(fontFamily: "catFont", fontSize: 30, fontWeight: FontWeight.bold,
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(children: [
                    ...
                    if (state is CatAnalysisSuccess)
                      _buildResultSection(isDark, state.catData, screenH, languageProvider),
  ...
  Widget _buildImageWithAnalyzeSection(
      bool isDark, File imageFile, LanguageProvider languageProvider, bool isProcessing) {
    return Padding(
      ...
      child: Column(children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(16),
              border: Border.all(
                  color: isDark ? Colors.grey[700]! : Colors.grey[300]!, width: 2)),
          child: Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(
                      color: isDark ? Colors.grey[600]! : Colors.grey[400]!, width: 2)),
              child: ClipRRect(
                  borderRadius: BorderRadius.circular(10),
        ...
        Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              label: Text(
                isProcessing
                    ? languageProvider.translate(en: 'Processing...', th: 'กำลังวิเคราะห์...')
                    : languageProvider.translate(en: 'Analyze Data', th: 'วิเคราะห์ข้อมูล'),
                style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
              ),
              style: ElevatedButton.styleFrom(
                  padding: const EdgeInsets.symmetric(vertical: 14),
  ...
  Widget _buildResultSection(
      bool isDark, CatData catData, double screenH, LanguageProvider languageProvider) {
    return Padding(
      padding: const EdgeInsets.all(16),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(width: 16),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _buildInfoRow(languageProvider.translate(en: 'Cat Color:', th: 'สีแมว:'),
                  catData.name, isDark),
              const SizedBox(height: 10),
              _buildInfoRow(languageProvider.translate(en: 'Age:', th: 'อายุ:'),
                  catData.age != null ? '${catData.age} years' : 'N/A', isDark),
              const SizedBox(height: 10),
              _buildInfoRow(languageProvider.translate(en: 'Breed:', th: 'พันธุ์:'),
                  catData.breed ?? 'N/A', isDark),
              const SizedBox(height: 10),
              _buildInfoRow(languageProvider.translate(en: 'Size:', th: 'ขนาด:'),
                  catData.sizeCategory, isDark),
            ])),
        ...
        const SizedBox(height: 20),
        Text(languageProvider.translate(en: 'Recommended Products', th: 'สินค้าแนะนำ'),
            style: TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.bold,
                color: isDark ? Colors.white : Colors.black87)),
  ...
  Widget _buildInfoRow(String label, String value, bool isDark) {
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(
          child: Text(value,
              style: TextStyle(
                  fontSize: 16, color: isDark ? Colors.white60 : Colors.black54))),
    ]);
  }
            ...
            Stack(children: [
              ...
              Positioned(
                ...
                child: GestureDetector(
                  onTap: () async {
                    if (isFav) {
                      await _favouriteApi.removeFromFavourite(clothingUuid: product['id']);
                      _showSuccessMessage(languageProvider.translate(
                          en: 'Removed from favourites', th: 'ลบออกจากรายการโปรดแล้ว'));
                    } else {
                      await _favouriteApi.addToFavourite(clothingUuid: product['id']);
                    ...
                    child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                          ...
                          shape: RoundedRectangleBorder(
                              borderRadius: BorderRadius.circular(8))),
                      child: Text(languageProvider.translate(en: 'Buy', th: 'ซื้อ'),
                          style: const TextStyle(fontSize: 11)),
                    ),
                  ),
                  const SizedBox(width: 4),
                  ElevatedButton(
                    onPressed: () => _showInfoMessage(languageProvider.translate(
                        en: 'Opening details...', th: 'กำลังเปิดรายละเอียด...')),
                    style: ElevatedButton.styleFrom(
                        padding:
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(
                    _isCapturing
                        ...
                        : languageProvider.translate(en: 'Take Photo', th: 'ถ่ายรูป'),
                    style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
                  ),
                  style: ElevatedButton.styleFrom(
                      padding: const EdgeInsets.symmetric(vertical: 14),
